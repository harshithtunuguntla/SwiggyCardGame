...
class Match { //Match class to conduct a match
  constructor() { //All the variablees required for the whole match are declared here
  ...
  initMatchVariables() { //initialise match variables such as number of players, cards for each player
    ...
    try{while (this.playersCount * this.initCards > 52) {
  ...
  createPlayers() { //Create players with their information
    try{for (let j = 0; j < this.playersCount; j++) { //To prompt and take user input for number of players and their details
  ...
  distributeCards() { //Distribute cards to the players
    try{for (let i = 0; i < this.players.length; i++) {
  ...
  initPlayerToBegin() { //Pick a player randomly to start the play
    try{let randomPlayer = Math.floor(Math.random() * this.playersCount);
  ...
  addCardOnDiscardPile(theCard) {
    try{this.discardPile.push(theCard);}
    catch(err){
  ...
  createDiscardPile() {
    try{this.addCardOnDiscardPile(this.deck[0]);}
    catch(err){
  ...
  discardPileTopCard() {
    try{return this.discardPile[this.discardPile.length - 1];}
    catch(err){
  ...
  availableCardsInDiscardPile() {
    try{return this.discardPile.length;}
    catch(err){
  ...
  turnToNextPlayer(skipCount) {
    try{this.currentPlayer = nextPlayer(
      this.players,
      this.currentPlayer,
      this.reverseMode,
      skipCount
  ...
  didPlayerWin() {
    try{return this.currentPlayer.hand.length == 0;}
    catch(err){
      console.log("Error occured while calulating if the player has any cards left")
      console.log(err)
    }
  }
  ...
  printPlayerStatus() {
    try{console.log("Current Player Name : " + this.currentPlayer.name);
    console.log(
  ...
  printCardDetails(message, card) {
    try{console.log(message + card.rank + " of " + card.suit);}
    catch(err){
  ...
  printStatus(message) {
    try{console.log("Status : " + message);}
    catch(err){
  ...
  actionForKing() { //Action to perform when the discarded card is king
    try{this.reverseMode = !this.reverseMode;
  ...
  actionForQueen() { //Action to perform when the discarded card is queen
    try{this.printStatus(this.currentPlayer.name + " discarded Queen card.");
  ...
  actionForJack() { //Action to perform when the discarded card is jack
    try{this.printStatus(this.currentPlayer.name + " discared Jack card.");
    this.turnToNextPlayer(0); //The Immediate Player
    this.printStatus( //Next player should draw 4 cards
      this.currentPlayer.name +
  ...
  actionForAce() { //Action to perform when the discarded card is ace
    try{this.printStatus(this.currentPlayer.name + " discared Ace card.");
    let playerSkipped = nextPlayer( //The immediate player
      this.players,
      this.currentPlayer,
      this.reverseMode,
      0
    ...
    this.turnToNextPlayer(1); }    //Next player, skipping the immediate player, skip 1
    catch(err){
  ...
  play() { //Main Game Play
    ...
    try {
      ...
      while (true) { //Until the game ends
        ...
        if (matchedCard == null) { //check if there is a matching card to discard
          this.printStatus(
          ...
          if (this.drawCards(1) == false) { //pick a card from the , if the deck is empty declare the game
        ...
        else { //If there is a matching card to discard
          this.printCardDetails("Card matching to discard : ", matchedCard);
          this.addCardOnDiscardPile(matchedCard);

          if (this.didPlayerWin() == true) { //Check if all the cards in the player hand are emptied after pushing on discard pile
            this.printStatus(
          ...
          if (matchedCard.rank == "King") { //Action to take for king card
            this.actionForKing();
          }

          else if (matchedCard.rank == "Queen") { //Action to take for queen card
            if (this.actionForQueen() == false) { //Check if the deck has two cards available and draw
              break;
            }
          }

          else if (matchedCard.rank == "Jack") { //Action to take for jack card
            if (this.actionForJack() == false) { //Check if the deck has four cards available and draw
              break;
            }
          }

          else if (matchedCard.rank == "Ace") { //Action to take for Ace card
            this.actionForAce();
          }
          
          else { //There is no action to take if it is not an action card
            this.printStatus(
